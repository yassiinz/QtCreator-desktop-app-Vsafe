...
QSqlQuery qry;
int i=0;
qry.exec("SELECT NUM,ETAT FROM PISTE");
while (qry.next())
{
...
QLinearGradient gradient(0, 0, 0, 400);
                  ...
                  amande->setStackingGap(1);
                   //couleurs
                  amande->setName("Repartition des numéro de piste selon la capacité ");
                  ...
                  // ajout des données  (statistiques de capacité)//

                  QVector<double> PlaceData;
                  QSqlQuery q1("SELECT CAPACITE FROM PISTE");
                  while (q1.next()) {
